...
function processAndRenderResults(apiData, requestedNos) {
    ...
    requestedNos.forEach(reqNo => {
        ...
        if (!item) {
            ...
            html += `<tr>
                <td style="font-weight:600; color:var(--primary);">${escapeHtml(reqNo)} <span class="barcode-trigger-btn" data-barcode="${escapeHtml(reqNo)}" title="显示 Code128 条形码" style="cursor: pointer; margin-left: 2px; vertical-align: middle; display: inline-flex; align-items: center;">${BARCODE_SVG}</span></td>
